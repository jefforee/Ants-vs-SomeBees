...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        self.range_count = self.min_range
        if self.min_range>0: #If min_range is greater than 0, then go through the entrances from 0 to min_range, and start checking after that value.
            for _ in range(0,self.min_range):
                if current_place == beehive:
                    return None
                current_place = current_place.entrance
        while current_place != beehive and (self.min_range <= self.range_count <= self.max_range):
            if rANTdom_else_none(current_place.bees):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor == 0:
            for each_bees in ant_place.bees[:]:
                each_bees.reduce_armor(self.damage)
        for each_bees in ant_place.bees[:]:
            each_bees.reduce_armor(amount)
        # END Problem 5
...
# END Problem EC
        ...
        if not self.is_a_queen:
            ...
            while current_place.exit:
                if current_place.exit.ant:
                    if current_place.exit.ant.is_damage_doubled == False:
                        if not current_place.exit.ant == FireAnt:
                            current_place.exit.ant.damage *= 2
                            current_place.exit.ant.is_damage_doubled = True
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor == 0 and self.is_a_queen:
            bees_win()
